    ...
    let toDeleteCount = parseInt(parts[1]);
        if (message.member.hasPermission('MANAGE_MESSAGES')) {
            if (message.guild.member(bot.user).hasPermission('MANAGE_MESSAGES')) {
                ...
                message.channel.sendMessage('Eu não tenho permissão necessária para fazer isso...').then(message => {
                                setTimeout(() => {message.delete()}, 5000)
                            })
            }
        } else {
            message.channel.sendMessage('Você não tem a permissão necessária para fazer isso...').then(message => {
                                setTimeout(() => {message.delete()}, 5000)
                            })
        }
}